...
访问 `https://<你的域名>/admin` 进入管理后台，可绕过房间密码查看/管理所有房间与消息。

### 配置管理密码

管理密码来自 Cloudflare 控制台配置的 `PASSWORD` 变量（**不要写入 wrangler.toml / 代码库**）：

```bash
# 方式一：加密 Secret（推荐，`wrangler secret put` 后自动生效）
wrangler secret put PASSWORD

# 方式二：Cloudflare 控制台 → Worker 设置 → Variables → 添加变量 PASSWORD
```

未配置 `PASSWORD` 时，管理 API 返回 503（页面提示"管理密码未配置"）。

### 功能

- 查看所有房间（密码状态、消息数、文件数、存储大小、创建时间）
- 查看任意房间消息（绕过房间密码）
- 删除任意房间 / 消息（绕过房间密码）

### 安全说明

- 密码以 SHA-256 哈希传输（明文不离开浏览器），服务端恒定时间比较
- 鉴权失败限流：同一 IP 每 5 分钟最多 10 次尝试
- 管理页面仅静态壳，真正的保护在 `/api/admin/*` 的密码鉴权
- 管理 API 响应禁用缓存（`no-store`）
